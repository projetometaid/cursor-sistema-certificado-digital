...
// Carregar variáveis de ambiente (suporta config/.env)
...
function consultarCPFAssertiva(cpf, token) {
    return new Promise((resolve, reject) => {
        ...
        // Endpoint com finalidade (LGPD)
        const qs = new URLSearchParams({ cpf: cleanCpf, idFinalidade: '1' }).toString();
        const options = {
            hostname: 'api.assertivasolucoes.com.br',
            port: 443,
            path: `/localize/v3/cpf?${qs}`,
            method: 'GET',
            headers: {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    const nome = cadastrais?.nome || response?.name || null;
                    const nascRaw = cadastrais?.dataNascimento || response?.birth_date || null;
                    const nascIso = normalizeBirthDate(nascRaw);
                    const encontrado = !!(nome || nascRaw);
                    ...
                    if (res.statusCode === 200) {
                        ...
                        resolve({
                            ...
                            encontrado: false,
                            dataNascimento: null,
                            erro: response?.message || `HTTP ${res.statusCode}`
                        });
                    }
                } catch (error) {

...
export default function CertificateNew(){
  ...
  async function onSubmit(e){
    ...
    try{
      if(product === 'ECPF_A1'){
        const body = { cpf, email, telefone, cep, numero, nome: nome||undefined, dataNascimento: dataNascimento||undefined };
        ...
        const body = { cnpj, cpfResponsavel, email, telefone, cepPessoa, numeroPessoa };
      ...
      <form onSubmit={onSubmit} className="grid gap-4 max-w-3xl">
        ...
        {product==='ECPF_A1' ? (
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              <label className="block text-sm mb-1" htmlFor="nome">Nome (opcional)</label>
              <input id="nome" className="w-full" value={nome} onChange={e=>setNome(e.target.value)} />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              <label className="block text-sm mb-1" htmlFor="dataNascimento">Data de nascimento (opcional)</label>
              <input id="dataNascimento" type="text" placeholder="YYYY-MM-DD ou DD/MM/YYYY" className="w-full" value={dataNascimento} onChange={e=>setDataNascimento(e.target.value)} />
            </div>
          </div>
        ) : (
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              <label className="block text-sm mb-1" htmlFor="cpfResponsavel">CPF do responsável</label>
            ...
            <div>
              <label className="block text-sm mb-1" htmlFor="cepPessoa">CEP (pessoa)</label>
              <input id="cepPessoa" className="w-full" value={cepPessoa} onChange={e=>setCepPessoa(e.target.value)} required />
            </div>
            <div>
              <label className="block text-sm mb-1" htmlFor="numeroPessoa">Número (pessoa)</label>
